...
class StepperMotor : public scheduler_task
{
    ...
    bool run(void *p)
    {
        ...
        if (xQueueReceive(startStep,&steps,portMAX_DELAY))
        {
			...
			{ // if Pin is high = forward = 1 on right LED
                LD.setRightDigit('1');
            }
            else
			{ // else pin is low and reverse direction = 0 on LED
			    LD.setRightDigit('0');
            }
			...
			if(QueueReceive(EndProcess,&Finished,portMAX_DELAY))
			{
				if(steps == 0)
					{
						...
						while(1)
						{
							...
							{ //Will start when button pressed;
								break;
							}
						}
...
class BluetoothTask : public scheduler_task
{
    ...
    bool run(void *p)
    {
		...
		{ // wait for start byte
            mlLiquid = 0;
            while (!done)
			{ // add liquid until end 'z' signal // grabs byte
                    ch = uart3_getchar();
                    switch(ch){
                             case 'b':
                             ...
                             case 'z': // End signal
								 mlLiquid += 0;
								 QueueSend(EndProcess, &Finished, portMAX_DELAY);
                                 done = true;
                                 break;
